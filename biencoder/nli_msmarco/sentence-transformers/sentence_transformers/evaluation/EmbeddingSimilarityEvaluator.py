...
class EmbeddingSimilarityEvaluator(SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1, num_proc: int = None) -> float:
        ...
        ### Due to mismatched_sizes_all_gather in encode() in SentenceTransformer convert_to_numpy does not work as tensors will be on the cpu > convert to tensor instead ###
        embeddings1 = model.encode(self.sentences1, batch_size=self.batch_size, show_progress_bar=self.show_progress_bar, convert_to_tensor=True, num_proc=num_proc)
        embeddings2 = model.encode(self.sentences2, batch_size=self.batch_size, show_progress_bar=self.show_progress_bar, convert_to_tensor=True, num_proc=num_proc)
