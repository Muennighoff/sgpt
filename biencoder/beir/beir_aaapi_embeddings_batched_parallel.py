...
def parse_args():
    ...
    parser.add_argument("--engine", type=str, default="13B_new_codebase_sum_magma_60k_symmetric_symmetric128", help="Model to use.")
    parser.add_argument(
        "--tokenizername",
        type=str,
...
class AAAPIRetriever:
    def __init__(
        self,
        engine="13B_new_codebase_sum_magma_60k_symmetric_symmetric128",
        tokenizer=None,
        endpoint="search",
        api_key=API_KEY,
        dataset="",
    ...
    def encode_queries(self, queries: List[str], batch_size: int, **kwargs) -> np.ndarray:
        ...
        if os.path.exists(embedding_queries_path):
            ...
            embeddings = self.embed(
                ...
                embedding_type="asymmetric_query",
    ...
    def embed(
        ...
        def call_aa_api(
            ...
            for j, row in enumerate(batch):
                ...
                try:
                    out.append(result["embeddings"][embedding_type]["weighted_mean"])
                except:
                    print("RES:", result)
            return out

        df_parallelizer = DataFrameParallelizer(
            function=call_aa_api,
